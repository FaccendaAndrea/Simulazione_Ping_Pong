  ...
  return (
    <div style={{minHeight:'100dvh',minWidth:'100vw',display:'flex',justifyContent:'center',alignItems:'center',background:'#f5f6fa'}}>
      <div style={{background:'#fff',padding:'2.5em 2em',borderRadius:'14px',boxShadow:'0 2px 24px #0002',width:'100%',maxWidth:400,display:'flex',flexDirection:'column',alignItems:'center'}}>
        ...
        <h2 style={{textAlign:'center',marginBottom:'0.5em',color:'#222',fontWeight:700}}>Crea un nuovo account</h2>
        <p style={{textAlign:'center',marginBottom:'1.5em',color:'#222',fontSize:'1em'}}>Compila i campi per registrarti</p>
        <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:'1.2em',width:'100%'}} autoComplete="on">
          <div style={{display:'flex',flexDirection:'column',gap:4}}>
            <label htmlFor="nome" style={{fontWeight:500,marginBottom:2,color:'#222'}}>Nome</label>
            <input id="nome" type="text" placeholder="Nome" value={nome} onChange={e => setNome(e.target.value)} required style={{padding:'0.7em',borderRadius:6,border:'1px solid #dfe6e9',outlineColor:'#0984e3',background:'#fff',color:'#222'}} autoFocus />
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:4}}>
            <label htmlFor="cognome" style={{fontWeight:500,marginBottom:2,color:'#222'}}>Cognome</label>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:4}}>
            <label htmlFor="email" style={{fontWeight:500,marginBottom:2,color:'#222'}}>Email</label>
            <input id="email" type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} required style={{padding:'0.7em',borderRadius:6,border:'1px solid #dfe6e9',outlineColor:'#0984e3',background:'#fff',color:'#222'}} />
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:4}}>
            <label htmlFor="password" style={{fontWeight:500,marginBottom:2,color:'#222'}}>Password</label>
        ...
        <p style={{marginTop:'1.5em',textAlign:'center',fontSize:'0.98em',color:'#222'}}>Hai già un account? <a href="/login" style={{color:'#0984e3',fontWeight:500,textDecoration:'underline'}}>Login</a></p>
      </div>
    </div>
  );
} 